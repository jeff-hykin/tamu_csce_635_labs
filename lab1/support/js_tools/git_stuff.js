...
export const hasPendingChanges = async ()=>{
    return (await run`git status --porcelain  ${Stdout(returnAsString)}`).split(/\n/g).filter(each=>each.length>0).length > 0
}

export const ensureNoChanges = async (context)=>{
    if (await hasPendingChanges()) {
        if (confirm(`${context}\nIt looks like you have pending git changes. Do you want me to stash your changes so I can continue?`)) {
            console.log(`Okay, stashing`)
            await run`git add -A`
            await run`git stash`
        } else {
            console.log(`Okay, once you've stashed your changes please re-run this`)
            throw Error(`Git changes are present`)
        }
    }
}

export const mergeUnrelated = async ({url, branch="master", commit})=> {
    await ensureNoChanges()
    await run`git remote remove @__temp__ ${Stdout(null)} ${Stderr(null)}`
    await run`git remote add @__temp__ ${url}`
    await run`git fetch @__temp__ ${branch}`
    if (commit) {
        await run`git cherry-pick ${commit}`
    } else {
        await run`git pull --allow-unrelated-histories @__temp__ ${branch}`
    }
...
export const gitUrl = async () => {
    return (await run`git config --get remote.origin.url ${Stdout(returnAsString)}`).replace(/\n/,"")
}