...
def battery_remaining():
    return f"{robot_state_client.get_robot_state().power_state.locomotion_estimated_runtime.seconds/60} min"
            ...
            params=spot_command_pb2.MobilityParams(
                body_control=spot_command_pb2.BodyControlParams(
                    ...
                    body_pose=spot_command_pb2.BodyControlParams.BodyPose(
                        ...
                        base_offset_rt_root=trajectory_pb2.SE3Trajectory(
                            points=[
                                trajectory_pb2.SE3TrajectoryPoint(
                                    time_since_reference=seconds_to_duration(2.5),
                                    pose=(odom_T_flat_body * math_helpers.SE3Pose(
                                        x=0.075, y=0, z=0, rot=math_helpers.Quat()
                                    )).to_proto(),
                                ), 
                                trajectory_pb2.SE3TrajectoryPoint(
                                    time_since_reference=seconds_to_duration(5.0),
                                    pose=(odom_T_flat_body * math_helpers.SE3Pose(
                                        x=0.0, y=0, z=0, rot=math_helpers.Quat(w=0.9848, x=0, y=0.1736, z=0)
                                    )).to_proto(),
                                ),
                                trajectory_pb2.SE3TrajectoryPoint(
                                    time_since_reference=seconds_to_duration(7.5),
                                    pose=(odom_T_flat_body * math_helpers.SE3Pose(
                                        x=0.0, y=0, z=0, rot=math_helpers.Quat()
                                    )).to_proto(),
                                )
                            ]
                        )
